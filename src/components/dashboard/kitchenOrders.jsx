...
const OrderTable = ({ list }) => (
  <>
    {/* Desktop */}
    <div className="hidden lg:block border border-gray-100 rounded-2xl overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody>
          {list.map((order, index) => (
            <tr
              key={order.id}
              className={index !== list.length - 1 ? "border-b border-gray-50" : ""}
            >
              ...
              <td className="px-5 py-3.5">
                <span className="bg-blue-50 text-blue-600 text-xs px-2.5 py-1 rounded-full capitalize">
                  {order.status}
                </span>
              </td>
            </tr>
    ...
    {/* Mobile */}
    <div className="lg:hidden divide-y divide-gray-100 rounded-xl border border-gray-100 bg-white">
      {list.map((order) => (
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-[#2CD377] text-sm">
                  #{order.id}
                </span>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.129a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.209-.503l4.493 1.498A1 1 0 0121 15.72V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"
              />
            </svg>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">
            {orders.length} scheduled orders today
          </p>
        </div>
      ...
      {generateResult && (
        <div className="bg-[#2CD377]/10 border border-[#2CD377]/20 rounded-2xl px-5 py-3 flex items-center gap-3">
          ...
          <p className="text-sm font-medium text-[#2CD377]">
            {generateResult.data.orders_created} orders created ·{" "}
            {generateResult.data.orders_assigned} assigned to{" "}
            {generateResult.data.riders_used} rider
          </p>
        </div>
      )}
          ...
          {lunch.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="bg-[#2CD377]/10 text-[#2CD377] text-xs font-bold px-2 py-0.5 rounded-full">
                  {lunch.length}
                </span>
              </div>
